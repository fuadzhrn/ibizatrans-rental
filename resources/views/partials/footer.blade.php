<footer class="ibiza-footer" id="contact">
    <div class="container footer-container">
        <div class="footer-brand">
            <div class="logo"> <span class="logo-gold">Ibiza</span> Trans</div>
            <p class="muted">Professional transportation and tour service provider based in Banyuwangi. Hospitality-oriented, reliable, and comfortable travel solutions.</p>
        </div>

        <div class="footer-links">
            <h4>Quick Links</h4>
            <ul>
                <li><a href="{{ route('home') }}">Home</a></li>
                <li><a href="#services">Layanan</a></li>
                <li><a href="#packages">Paket Tour</a></li>
                <li><a href="#contact">Contact</a></li>
            </ul>
        </div>

        <div class="footer-contact">
            <h4>Contact</h4>
            <p>WhatsApp / Call: <a href="https://wa.me/6285703399966">+62 857-0339-9966</a></p>
            <p>Booking Trip: <a href="https://wa.me/6289515324763">+62 895-1532-4763</a></p>
            <p>Address: Jalen I, Setail, Kec. Genteng, Kabupaten Banyuwangi, Jawa Timur 68465</p>
            
        </div>

        <div class="footer-cta">
            <h4>Ready to book?</h4>
            <a class="btn btn-book-outline" href="https://wa.me/6285703399966?text=Halo%20Ibiza%20Trans%2C%20saya%20ingin%20bertanya%20tentang%20layanan%20rental%20mobil%20dan%20paket%20tour." target="_blank">Booking via WhatsApp</a>

            <div class="footer-social">
                <h4>Social Media</h4>
                <ul>
                    <li><a href="https://instagram.com/ibizatrans" target="_blank" rel="noopener"><i class="ri-instagram-line" aria-hidden="true"></i> Instagram</a></li>
                    <li><a href="https://www.tiktok.com/@ibizatrans" target="_blank" rel="noopener"><i class="ri-tiktok-line" aria-hidden="true"></i> TikTok</a></li>
                    <li><a href="https://wa.me/6285703399966" target="_blank" rel="noopener"><i class="ri-whatsapp-line" aria-hidden="true"></i> WhatsApp</a></li>
                </ul>
            </div>
        </div>
    </div>

    <div class="footer-bottom">
        <div class="container">&copy; {{ date('Y') }} Ibiza Trans — All rights reserved.</div>
    </div>
</footer>
